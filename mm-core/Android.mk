...
ifeq ($(filter $(PRODUCT_BOARD_PLATFORM), msmnile),$(PRODUCT_BOARD_PLATFORM))
OMXCORE_CFLAGS += -D_NILE_
else ifeq ($(filter $(PRODUCT_BOARD_PLATFORM), $(MSMSTEPPE)),$(PRODUCT_BOARD_PLATFORM))
OMXCORE_CFLAGS += -D_STEPPE_
else ifeq ($(filter $(PRODUCT_BOARD_PLATFORM), $(TRINKET)),$(PRODUCT_BOARD_PLATFORM))
OMXCORE_CFLAGS += -D_TRINKET_
else ifeq ($(filter $(PRODUCT_BOARD_PLATFORM), atoll),$(PRODUCT_BOARD_PLATFORM))
...
LOCAL_C_INCLUDES        += $(TOP)/hardware/qcom/media/sm8150/libplatformconfig
...
ifneq (,$(filter msmnile sdmshrike $(MSMSTEPPE) $(TRINKET) atoll,$(PRODUCT_BOARD_PLATFORM)))
